...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleIllegalArgumentException(IllegalArgumentException ex) {
        ...
        ExceptionResponseDTO response = new ExceptionResponseDTO("FAILURE", ex.getMessage());
    ...
    public ResponseEntity<?> handleMethodArgumentNotValidException(MethodArgumentNotValidException ex) {
        ...
        if (errorMessages.size() == 1) {
            ...
            ExceptionResponseDTO response = new ExceptionResponseDTO("FAILURE", errorMessages.get(0));
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        } else {
    ...
    public ResponseEntity<?> handleHttpMessageNotReadableException(HttpMessageNotReadableException ex) {
        ...
        ExceptionResponseDTO response = new ExceptionResponseDTO("FAILURE", "요청 본문이 비어있거나 올바르지 않습니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        ExceptionResponseDTO response = new ExceptionResponseDTO("FAILURE", "필수 데이터가 누락되었거나 잘못된 데이터가 입력되었습니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleServerError(Exception ex) {
        ...
        ExceptionResponseDTO response = new ExceptionResponseDTO("FAILURE", "서버 내부 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
